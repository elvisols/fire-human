import React, { Component } from "react";
import { Link } from "react-router-dom";
import PropTypes from "prop-types";
import { connect } from "react-redux";
import { deletePerson } from "../../actions/personActions";

class PersonList extends Component {
  onDeleteClick = id => {
    this.props.deletePerson(id);
  };
  render() {
    const { psn } = this.props;
    return (
      <div className="container">
        <div className="card card-body bg-light mb-3">
          <div className="row">
            <div className="col-2">
              <span className="mx-auto">{psn.id}</span>
            </div>
            <div className="col-lg-6 col-md-4 col-8">
              <h3>{psn.first_name + " " + psn.last_name}</h3>
              <p>. . .</p>
            </div>
            <div className="col-md-4 d-none d-lg-block">
              <ul className="list-group">
                <Link to={`/viewPerson/${psn.id}`}>
                  <li className="list-group-item board">
                    <i className="fa fa-flag-checkered pr-1">
                      {" "}
                      View {psn.first_name}'s Detail{" "}
                    </i>
                  </li>
                </Link>
                <Link to={`/editPerson/` + psn.id}>
                  <li className="list-group-item update">
                    <i className="fa fa-edit pr-1">
                      {" "}
                      Update {psn.first_name}'s Detail
                    </i>
                  </li>
                </Link>

                <li
                  className="list-group-item delete"
                  onClick={this.onDeleteClick.bind(this, psn.id)}
                >
                  <i className="fa fa-minus-circle pr-1">
                    {" "}
                    Delete {psn.first_name}
                  </i>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    );
  }
}

PersonList.propTypes = {
  deletePerson: PropTypes.func.isRequired
};

export default connect(null, { deletePerson })(PersonList);
